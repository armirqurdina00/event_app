...
describe('Tests for groups endpoints.', function () {
  ...
  it('POST /v1/groups/{group_id}/joins', async function () {
    const body: GroupReqBody = {
      title: 'Street Salsa',
      description: 'City Park',
      link: 'https://example.com/whatsapp-group',
    ...
    const { group_id, numberOfJoins }: GroupRes = await backend_client.groups.postGroups(user_id, body);
    group_ids.push(group_id);

    expect(numberOfJoins).to.equal(1);

    for (let i = 2; i <= 5; i++) {
      const response: GroupRes = await backend_client.groups.postGroupsJoins(group_id);
      expect(response.numberOfJoins).to.equal(i);
    }
  });
  ...
  it('GET /v1/groups returns only groups within the specified radius.', async function () {
    ...
    const bodyOutsideRadius: GroupReqBody = {
      ...
      coordinates: {
        ...centerCoordinates,
        latitude: centerCoordinates.latitude + 0.1, // adjust to ensure outside the radius
      },
    };
  ...
  it('GET /v1/groups ordered by user_location', async function () {
    ...
    const createGroupWithRandomCoordinates = async () => {
      const coords = COORDINATES[Math.floor(Math.random() * COORDINATES.length)];
    ...
    response.items.forEach(item => {
      const currentDistance = getDistanceBetweenPoints(COORDINATES[0], [
