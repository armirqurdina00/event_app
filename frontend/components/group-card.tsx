...
const GroupCard = ({ group }: { group: GroupRes }) => {
	...
	return (
		<div className='mx-3 w-96 rounded-xl border border-gray-200 bg-white shadow'>
			...
			>
				{clicked && (
					<div className='absolute bottom-0 left-0 right-0 top-0 z-10 h-full w-full overflow-hidden rounded-xl bg-white bg-fixed opacity-80 transition duration-300 ease-in-out '></div>
				)}
				{clicked && group.created_by === user?.sub && (
					<div className='absolute left-0 top-0 z-30 flex h-full w-full items-center justify-around p-2'>
						<LaunchIcon
							color='primary'
				...
				{clicked && group.created_by !== user?.sub && (
					<div className='absolute left-0 top-0 z-30 flex h-full w-full items-center justify-center p-2'>
						<LaunchIcon
							color='primary'
							onClick={handle_link_click}
							className='cursor-pointer text-4xl'
