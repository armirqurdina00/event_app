...
import { GroupJoinRes, GroupRes, GroupType } from '../utils/backend_client'
...
const GroupCard = ({ group }: { group: GroupRes }) => {
	...
	const handleJoin = async () => {
		...
		try {
			...
			try {
				res = await axios.get<any, AxiosResponse<GroupJoinRes>>(
					`/api/users/${user.sub}/groups/${group.group_id}/joins`
				)
			} catch (err) {
				if (err.response?.status === 404) {
					res = await axios.post<any, AxiosResponse<GroupJoinRes>>(
						`/api/users/${user.sub}/groups/${group.group_id}/joins`
					)
				} else {
					throw err
