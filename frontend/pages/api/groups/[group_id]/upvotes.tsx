  ...
  try {
    ...
    const backend_client = new BackendClient({
      ...
      TOKEN: access_token,
		...
		if (req.method === 'POST') {
			const response = await backend_client.groups.postUpvotes(
				String(req.query.group_id)
			)

			res.status(200).json(response)
		} else if (req.method === 'DELETE') {
			const response = await backend_client.groups.deleteUpvotes(
				String(req.query.group_id)
			)

			res.status(200).json(response)
		} else if (req.method === 'GET') {
			const response = await backend_client.groups.getUpvotes(
				String(req.query.group_id)
			)
