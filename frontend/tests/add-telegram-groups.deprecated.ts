...
test.skip('create telegram groups and save invite URLs', async ({
  browser,
}) => {
...
async function createAndRetrieveURL(navigationPage, groupName) {
  ...
  writeFileSync(
    inviteURLFilePath,
    JSON.stringify({ ...existingInviteURLs, ...inviteURLs }, null, 4)
  );
  console.log(
    `Extracted Telegram Invite URL for ${completeGroupName}:`,
    telegramInviteURL
  );
}
